...
public:
	void HelpCommand(const char *helpStr) {
		...
		if(strcasecmp("connect", helpStr) == 0) {
			...
			WriteOut("Winbox session.  The \"address\" parameter specifies the IP address or host name\n");
		...
		if(strcasecmp("startserver", helpStr) == 0) {
			WriteOut("IPXNET STARTSERVER starts and IPX tunneling server on this Winbox session.  By\n");
			WriteOut("default, the server will accept connections on UDP port 213, though this can be\n");
			WriteOut("changed.  Once the server is started, Winbox will automatically start a client\n");
		...
		if(strcasecmp("stopserver", helpStr) == 0) {
			WriteOut("IPXNET STOPSERVER stops the IPX tunneling server running on this Winbox\nsession.");
		...
		if(strcasecmp("status", helpStr) == 0) {
			WriteOut("IPXNET STATUS reports the current state of this Winbox's sessions IPX tunneling\n");
	...
	void Run(void)
	{
		WriteOut("IPX Tunneling utility for Winbox\n\n");
		if(!cmd->GetCount()) {
