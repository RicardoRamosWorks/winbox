...
//	else if (mtype == "vga_pvga1a")   { svgaCard = SVGA_ParadisePVGA1A; }
	...
	else E_Exit("Winbox:Unknown machine type %s",mtype.c_str());
}
...
void DOSBOX_Init(void) {
	...
	secprop=control->AddSection_prop("Winbox",&DOSBOX_RealInit);
	Pstring = secprop->Add_path("language",Property::Changeable::Always,"");
	Pstring->Set_help("Select another language file.");

	Pstring = secprop->Add_string("machine",Property::Changeable::OnlyAtStart,"svga_s3");
	Pstring->Set_values(machines);
	Pstring->Set_help("The type of machine Winbox tries to emulate.");
	...
	Pint->SetMinMax(0,10);
	Pint->Set_help("How many frames Winbox skips before drawing one.");
	...
	Pmulti_remain->Set_help(
		"Amount of instructions Winbox tries to emulate each millisecond.\n"
	...
	Pstring = Pmulti_remain->GetSection()->Add_string("type",Property::Changeable::Always,"fixed 25000");
	...
	Pbool->Set_help("Allow the soundblaster mixer to modify the Winbox mixer.");
	...
	Pstring->Set_help(
		...
		"(Remember to reset Winbox's mapperfile if you saved it earlier)");
...
//	secprop->AddInitFunction(&CREDITS_Init);
	...
	MSG_Add("CONFIGFILE_INTRO",
	        "# This is the configuration file for Winbox %s. (Please use the latest version of Winbox)\n"
	        "# Lines starting with a # are comment lines and are ignored by Winbox.\n"
