...
BatchFile::BatchFile(DOS_Shell *host, char const *const resolved_name,
	...
	DOS_Canonicalize(resolved_name,
	                 totalname); // Get fullname including drive specificiation
	...
	do {
		...
		if (n > 0) {
			...
			if (c > 31 || c == 0x1b || c == '\t' || c == 8) {
				// Only add it if room for it (and trailing zero) in the buffer,
				// but do the check here instead at the end So we continue
				// reading till EOL/EOF
	...
	while (*cmd_read) {
		if (*cmd_read == '%') {
			...
			if (next > '0' && next <= '9') {
				...
				if (!first) {
					/* *cmd_write++ = '%';*/
					// check if input contains cycles + max/auto  and that next
					// character is space or empty If so, don't ignore it. This
					// way cycles can still be set from within batch files
					char peak = *(cmd_read);
					if (peak == 0 || peak == ' ' || peak == '\r' ||
					        peak == '\n') {
						strncpy(temp_cycles_hack, temp, cmd_read - temp);
						temp_cycles_hack[cmd_read - temp] = 0;
						upcase(temp_cycles_hack);
						const char *cycles_test_cycles =
						    strstr(temp_cycles_hack, "CYCLES");
						if (cycles_test_cycles) {
							const char *cycles_test_max =
							    strstr(cycles_test_cycles, "MAX");
							const char *cycles_test_auto =
							    strstr(cycles_test_cycles, "AUTO");
...
bool BatchFile::Goto(char *where) {
	...
	if (!DOS_OpenFile(filename.c_str(), (DOS_NOT_INHERIT | OPEN_READ),
	                  &file_handle)) {
		LOG(LOG_MISC, LOG_ERROR)
		("SHELL:Goto Can't open BatchFile %s", filename.c_str());
		delete this;
		return false;
	}
