...
void DOS_Shell::DoCommand(char *line) {
	...
	while (*line) {
		...
		if (*line == '=')
			break;
		//		if (*line == ':') break; //This breaks drive switching as
		//that is handled at a later stage.
		if ((*line == '.') ||
	...
	*cmd_write = 0;
	...
	if (Execute(cmd_buffer, line))
		return;
	if (CheckConfig(cmd_buffer, line))
		return;
...
void DOS_Shell::CMD_DELETE(char *args) {
	...
	/* If delete accept switches mind the space infront of them. See the dir /p
	 * code */
...
void DOS_Shell::CMD_RENAME(char *args) {
	...
	if ((strchr(args, '*') != NULL) || (strchr(args, '?') != NULL)) {
		WriteOut(MSG_Get("SHELL_CMD_NO_WILD"));
		return;
	}
	...
	if (slash) {
		...
		char dir_source[DOS_PATHLENGTH + 4] = {
			0
		}; // not sure if drive portion is included in pathlength
		...
		*dummy = 0;
		...
		// dir_source and target are introduced for when we support multiple
		// files being renamed.
...
void DOS_Shell::CMD_CHDIR(char *args) {
	...
	if (!*args) {
		...
		unsigned char targetdisplay =
		    *reinterpret_cast<unsigned char *>(&args[0]);
		...
		/* Changedir failed. Check if the filename is longer then 8 and/or
		 * contains spaces */
...
void DOS_Shell::CMD_DIR(char *args) {
	...
	if (!strrchr(args, '*') && !strrchr(args, '?')) {
		Bit16u attribute = 0;
		if (DOS_GetFileAttr(args, &attribute) &&
		        (attribute & DOS_ATTR_DIRECTORY)) {
			strcat(args,
			       "\\*.*"); // if no wildcard and a directory, get its files
		}
	}
	if (!strrchr(args, '.')) {
	...
	*(strrchr(path, '\\') + 1) = 0;
	...
	do { /* File name and extension */
		DtaResult result;
		dta.GetResult(result.name, result.size, result.date, result.time,
		              result.attr);
			...
			if (attr & DOS_ATTR_DIRECTORY) {
				if (optW) {
					...
					WriteOut("%-8s %-3s   %-16s %02d-%02d-%04d %2d:%02d\n",
					         name, ext, "<DIR>", day, month, year, hour,
					         minute);
				}
				dir_count++;
			} else {
				if (optW) {
					...
					WriteOut("%-8s %-3s   %16s %02d-%02d-%04d %2d:%02d\n", name,
					         ext, numformat, day, month, year, hour, minute);
				}
				file_count++;
				byte_count += size;
			}
...
void DOS_Shell::CMD_COPY(char *args) {
	...
	// command line) Concatenating files go as follows: All parts except for the
	// last bear the concat flag. This construction allows them to be counted
	// (only the non concat set)
	char *source_p = NULL;
	...
	Bit32u count = 0;
	while (sources.size()) {
		/* Get next source item and keep track of old source for concat start
		 * end */
		oldsource = source;
		source = sources[0];
		...
		bool ret = DOS_FindFirst(const_cast<char *>(source.filename.c_str()),
		...
		while (ret) {
			...
			if ((attr & DOS_ATTR_DIRECTORY) == 0) {
				...
				if (DOS_OpenFile(nameSource, 0, &sourceHandle)) {
					...
					// Special variable to ensure that copy * a_file, where
					// a_file is not a directory concats.
					bool special =
					    second_file_of_current_source && target_is_file;
					...
					if (oldsource.concat ||
						...
						if (!oldsource.concat ||
						        (DOS_OpenFile(nameTarget, OPEN_READWRITE,
						                      &targetHandle) &&
						         DOS_SeekFile(targetHandle, &dummy,
						                      DOS_SEEK_END))) {
							// Copy
							static Bit8u
							buffer[0x8000]; // static, otherwise stack
							// overflow possible.
							bool failed = false;
...
void DOS_Shell::CMD_DATE(char *args) {
	...
	for (Bitu i = 0; i < 5; i++) {
		if (i == 1 || i == 3) {
			...
			if (formatstring[i] == 'M')
				bufferptr += sprintf(buffer + bufferptr, "%02u", (Bit8u)reg_dh);
			if (formatstring[i] == 'D')
				bufferptr += sprintf(buffer + bufferptr, "%02u", (Bit8u)reg_dl);
			if (formatstring[i] == 'Y')
				bufferptr +=
				    sprintf(buffer + bufferptr, "%04u", (Bit16u)reg_cx);
		}
	}
	WriteOut("%s %s\n", day, buffer);
	if (!dateonly)
		WriteOut(MSG_Get("SHELL_CMD_DATE_SETHLP"));
};
...
void DOS_Shell::CMD_TIME(char *args) {
	...
	if (ScanCMDBool(args, "H")) {
		...
		Bit32u ticks =
		    (Bit32u)(((double)(loctime->tm_hour * 3600 + loctime->tm_min * 60 +
		                       loctime->tm_sec)) *
		             18.206481481);
	...
	if (args && *args) {
		...
		if (!*args && !*word) { // Reset
			...
			dos.version.minor = 0;
		           ...
		           (strchr(word, '.') != 0)) { // Allow: ver set 5.1
		...
		WriteOut(MSG_Get("SHELL_CMD_VER_VER"), VERSION, dos.version.major,
		         dos.version.minor);
}