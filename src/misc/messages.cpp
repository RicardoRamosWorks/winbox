...
void MSG_Add(const char *_name, const char *_val) {
	...
	for (itmb tel = Lang.begin(); tel != Lang.end(); tel++) {
		if ((*tel).name == _name) {
			//			LOG_MSG("double entry for %s",_name); //Message file might
			//be loaded before default text messages
			return;
		}
	}
...
static void LoadMessageFile(const char *fname) {
	...
	/* This should never happen and since other modules depend on this use a
	 * normal printf */
	if (!mfile) {
	...
	while (fgets(linein, LINE_IN_MAXLEN, mfile) != 0) {
		...
		*writer = 0;
		...
		if (linein[0] == ':') {
			...
			if (ll && string[ll - 1] == '\n')
				string[ll - 1] =
				    0; // Second if should not be needed, but better be safe.
