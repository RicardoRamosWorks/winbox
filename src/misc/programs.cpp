	...
	while (presult != P_NOPARAMS) {
		...
		case P_WRITECONF_PORTABLE:
			...
			if (pvars.size() > 1)
				...
				// directory
				if (control->configfiles.size())
					writeconf(std::string("Winbox.conf"), false);
				else
					WriteOut(MSG_Get("PROGRAM_CONFIG_NOCONFIGFILE"));
			}
			break;
...
void PROGRAMS_Init(Section * /*sec*/) {
	...
	MSG_Add("PROGRAM_CONFIG_CONFDIR",
	        "Winbox %s configuration directory: \n%s\n\n");
	...
	MSG_Add("PROGRAM_CONFIG_HLP_SECTLIST",
	        "Winbox configuration contains the following sections:\n\n");
	...
	MSG_Add(
	    ...
	    "\nWinbox was started with the following command line parameters:\n%s");
