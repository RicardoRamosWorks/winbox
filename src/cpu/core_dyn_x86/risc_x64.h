...
#else
enum {
	// (high)byte-accessible - otimizado para acesso byte rápido
	X64_REG_RBX,
	X64_REG_RCX,
	X64_REG_RDX,
	X64_REG_RAX,
	...
	X64_REG_R10,
	X64_REG_R11,
	// non-volatiles - preservados através de chamadas
	X64_REG_R12,
	X64_REG_R13,
	X64_REG_R14,
	X64_REG_R15,
...
public:
	...
	opcode& setimm(Bit64u _imm, int size) {
		imm = _imm;
		imm_size = size;
		return *this;
	}
...
private:
	...
	/* EmitImm inline para performance */
	inline void EmitImm(void) {
		switch(imm_size) {
		case 1:
		...
		if (modrm < 0xC0) {
			...
			case 0:
				if ((modrm & 7) == 5) {
					...
					break;
			case 2:
				cache_addd((Bit32u)offset);
				break;
			case 1:
				cache_addb((Bit8u)offset);
				break;
			}
		}
		EmitImm();
...
static BlockReturn gen_runcodeInit(const Bit8u *code) {
	...
	/* Configura frame pointer */
	if ((Bit32u)(Bitu)&cpu_regs == (Bitu)&cpu_regs)
	...
	/* Carrega flags */
	opcode(0).setea(5, -1, 0, offsetof(CPU_Regs, flags)).Emit8(0x8B);
#if defined(_WIN64)
	...
	opcode(0).setea(4, -1, 0, CALLSTACK).Emit8(0x89);  // mov [rsp+...], eax
	opcode(4).setrm(ARG0_REG).Emit8(0xFF);   // jmp ARG0

	cache_addd((Bit32u)(cache.pos - diff - 4), diff);

	/* Epílogo */
	opcode(1).setea(5, -1, 0, offsetof(CPU_Regs, flags)).Emit8(0x33);
	opcode(4).setrm(1).setimm(FMASK_TEST, 4).Emit8(0x81);
	opcode(1).setea(5, -1, 0, offsetof(CPU_Regs, flags)).Emit8(0x31);

	opcode(4).set64().setea(4, -1, 0, CALLSTACK).Emit8(0x8B);
#if defined(_WIN64)
...
static GenReg * FindDynReg(DynReg * dynreg, bool stale = false) {
	...
	if (dynreg->flags & DYNFLG_HAS8) {
		/* Precisa de RAX, RBX, RCX, RDX para byte ops */
		...
		/* Scan reverso - preserva registradores de argumentos */
...
static void gen_dop_byte(DualOps op,DynReg * dr1,Bitu di1,DynReg * dr2,Bitu di2) {
	...
	case DOP_AND:
		tmp=0x22;
		if ((dr1==dr2) && (di1==di2)) goto nochange;
		break;
	case DOP_OR:
		tmp=0x0a;
		if ((dr1==dr2) && (di1==di2)) goto nochange;
...
static void gen_call_write(DynReg * dr,Bit32u val,Bitu write_size) {
	...
	switch (write_size) {
	case 1:
		func = (void*)mem_writeb_checked;
		break;
	case 2:
		func = (void*)mem_writew_checked;
		break;
	case 4:
		func = (void*)mem_writed_checked;
		break;
	default:
...
static void gen_return(BlockReturn retcode) {
	gen_protectflags();
	opcode(1).setea(4, -1, 0, CALLSTACK).Emit8(0x8B); // mov ecx, [rsp+...]
	opcode(0).set64().setrm(4).setimm(CALLSTACK + 8, 1).Emit8(0x83);
	if (retcode == 0)
		cache_addw(0xc033);  // xor eax, eax - quebra dependência
	else {
		cache_addb(0xb8);
		cache_addd(retcode);
	}
	opcode(4).setea(4, -1, 0, CALLSTACK - 8).Emit8(0xFF); // jmp [rsp+...]
}
