...
public:
	...
	/* Optimized Load - Pentium M benefits from fewer memory ops */
	void Load(DynReg * _dynreg, bool stale = false) {
		...
		if ((!stale) && (dynreg->flags & (DYNFLG_LOAD | DYNFLG_ACTIVE))) {
			/* Pentium M: mov reg, [mem] has 2 cycle latency from L1 */
			cache_addw(0x058b + (index << (8 + 3)));
			cache_addd((Bit32u)dynreg->data);
		}
		dynreg->flags |= DYNFLG_ACTIVE;
	}

	/* Optimized Save - combine with next instruction when possible */
	void Save(void) {
		if (GCC_UNLIKELY(!((Bitu)dynreg))) IllegalOption("GenReg->Save");
		dynreg->flags &= ~DYNFLG_CHANGED;
		cache_addw(0x0589 + (index << (8 + 3)));
		cache_addd((Bit32u)dynreg->data);
	}

	/* Release with early-out for unchanged registers */
	void Release(void) {
...
static GenReg * FindDynReg(DynReg * dynreg, bool stale = false) {
	...
	/* Fast path: register already allocated */
	if (dynreg->genreg) {
		dynreg->genreg->last_used = x86gen.last_used;
		return dynreg->genreg;
	}

	/* Find best register - Pentium M benefits from keeping EAX/ECX/EDX free for arithmetic */
	Bits i;
	Bits first_used = -1;
	Bits first_index;

	if (dynreg->flags & DYNFLG_HAS8) {
		/* 8-bit ops need EAX, EBX, ECX, EDX */
		...
		/* Scan backwards to preserve EAX for return values */
...
static inline GenReg * ForceDynReg(GenReg * genreg, DynReg * dynreg) {
...
static void gen_needflags(void) {
	if (!x86gen.flagsactive) {
		...
		cache_addb(0x9d);		// POPFD - 6 cycles on Pentium M
	}
}
...
static void gen_discardflags(void) {
	if (!x86gen.flagsactive) {
		x86gen.flagsactive = true;
		cache_addw(0xc483);		// ADD ESP,4 - uses stack engine
		cache_addb(0x4);
	}
}
...
static void gen_needcarry(void) {
	if (!x86gen.flagsactive) {
		x86gen.flagsactive = true;
		/* Pentium M: SHR [ESP],1 is faster than stack manipulation */
		cache_addw(0x2cd1);			// SHR DWORD [ESP],1
		cache_addb(0x24);
		cache_addd(0x0424648d);		// LEA ESP,[ESP+4] - uses AGU
	}
}
...
static void gen_dop_word(DualOps op, bool dword, DynReg * dr1, DynReg * dr2) {
	...
	case DOP_XCHG:
		if (dr1 == dr2) return;
		dr2->flags |= DYNFLG_CHANGED;
		/* Pentium M: xchg is faster with same-size registers */
...
static void gen_dop_word_imm(DualOps op, bool dword, DynReg * dr1, Bits imm) {
	...
	case DOP_TEST:
		...
		goto nochange;
	case DOP_MOV:
		/* Pentium M: MOV reg, imm is 1 cycle */
...
static void gen_call_write(DynReg * dr,Bit32u val,Bitu write_size) {
	...
	case 1:
		...
		break;
	case 2:
		cache_addd((Bit32u)mem_writew_checked - (Bit32u)cache.pos-4);
		break;
	case 4:
...
/* Optimized return - Pentium M has excellent return stack buffer
*/
static void gen_return(BlockReturn retcode) {
	gen_protectflags();
	cache_addb(0x59);			// POP ECX - restore flags
	if (retcode == 0) {
		cache_addw(0xc033);		// XOR EAX, EAX - breaks dependency chain
	} else {
		cache_addb(0xb8);		// MOV EAX, retcode
		cache_addd(retcode);
	}
	cache_addb(0xc3);			// RET - predicted by RSB
}
...
static void gen_return_fast(BlockReturn retcode, bool ret_exception = false) {
	...
	cache_addw(0x0d8b);			// MOV ECX, [flags]
...
/* FPU save - Pentium M FPU is not pipelined, minimize state saves */
